...
def test_email_verification_flow(page: Page):
    ...
    try:
        otp = get_latest_otp_imap(
            ...
            password=IMAP_PASS,
            email_to=test_email,
            timeout_sec=60  # Wait up to 60 seconds for the email
        )
        
        # 6. Input OTP
        otp_input = page.locator("input#emailOtp")
        otp_input.fill(otp)
        
        # 7. Click Check button
        check_button = page.locator("button:has-text('Check')")
        expect(check_button).to_be_enabled()
        check_button.click()
        
        # 8. Verify success message
        success_icon = page.locator("svg[class*='h-5 w-5']")
        success_text = page.locator("text=Email successfully verified!")
        expect(success_icon).to_be_visible()
